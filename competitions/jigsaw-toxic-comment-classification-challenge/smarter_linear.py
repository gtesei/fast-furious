...
def build_data_set(ngram=3,stem=False,max_features=2000,min_df=2,remove_stopwords=True):
    ...
    for i in range(train.shape[0]):
        ttext , upper_perc = clean_text(train["comment_text"][i])
    ...
    for i in range(test.shape[0]):
        ttext , upper_perc = clean_text(test["comment_text"][i])
        upper_percs.append( upper_perc )
        clean_train_comments.append( ttext )
    del test 
    ...
    if not stem:
        ...
        X = np.hstack([X,upper_percs_arr.reshape((X.shape[0],1))])
        X_train = X[:train_obs]
        X_test = X[train_obs:]
    else:
