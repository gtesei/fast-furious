...
X_t, X_te, y , embedding_matrix = pre_process_pre_trained_embed(train=train,test=test,we_fn='glove.6B.300d.txt')
model = get_model_bidirectional_avg(embed_size = EMBEDDING_DIM , embedding_matrix = embedding_matrix)
print(model.summary())
...
sample_submission.to_csv("sub_gru5_emb_marks.csv.gz", index=False , compression='gzip')