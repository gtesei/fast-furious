...
Xtrain$month <- as.integer(format(Xtrain$Date, "%m"))
Xtrain$year <- as.integer(format(Xtrain$Date, "%y"))
Xtrain$day <- as.integer(format(Xtrain$Date, "%d"))
Xtrain$Date <- NULL

Xtest$month <- as.integer(format(Xtest$Date, "%m"))
Xtest$year <- as.integer(format(Xtest$Date, "%y"))
Xtest$day <- as.integer(format(Xtest$Date, "%d"))
Xtest$Date <- NULL
...
write.csv(Xtrain,
          quote=FALSE, 
          file=paste0(ff.getPath("elab"),"Xtrain2.csv"),
          row.names=FALSE)
write.csv(Xtest,
          quote=FALSE, 
          file=paste0(ff.getPath("elab"),"Xtest2.csv"),
          row.names=FALSE)
