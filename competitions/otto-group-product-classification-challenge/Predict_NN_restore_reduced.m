...
%% loading data sets
printf("|--> loading data set reduced ...\n");
...
%% recovering grid 
printf("|--> recovering grid reduced ...\n");
grid = dlmread([curr_dir "/NN__main_grid_reduced.mat"]);
disp(grid);

nClass = max(grid(:,1))+1;
printf("|--> next class to process: Class%i ...\n",nClass);

%% recovering probability vectors
probs_vect = zeros(size(test_data,1) , length(y_class) );
printf("**************** recovering probability vectors [reduced] ...\n");
tic();
for j = 1:(nClass-1) 
  printf(">>> recovering Class%i  ..  \n", j );
  ytrain_j = ( Y == j  );
  ...
  %% recovering best parameters from the grid 
  p_opt_acc = grid(j,2);
  h_opt_acc = grid(j,3);
  lambda_opt_acc = grid(j,4);
  acc_opt = grid(j,5);
...
%% finishing remaining classes 
printf("*************  Finishing remaining classes [reduced] ...\n");
tic();
for j = nClass:length(y_class) 
