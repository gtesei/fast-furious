...
cat(">> loading from disk clusters queries / my_cluster_knn ... \n")
load(file=paste(getBasePath("data") , "queries" , sep=''))
load(file=paste(getBasePath("data") , "my_cluster_knn" , sep=''))

queries_2 = ls(q_map)
### check 
for (i in 1:length(queries_2)) if (queries_2[i] != queries[i] ) stop("queries_2[i] != queries[i]") 
####
...
#### cluter 
#cat(">> loading from disk clusters ... \n")
#load(file=paste(getBasePath("data") , "my_cluster.csv" , sep=''))
...
last_cluter_before_crack = max(cv_grid[ ! is.na(cv_grid$train_obs) , ]$query)
cat(">>> restoring from cluster N.",last_cluter_before_crack,"...\n")

## continue the job 
for (i in last_cluter_before_crack:cluster_card) {
