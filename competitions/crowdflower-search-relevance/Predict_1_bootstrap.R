...
get_labels = function(label_j, std , bootrap_factor) {
  if (bootrap_factor != 4) stop("only bootrap_factor == 4 supported at the moment")
  grid = expand.grid(x1 = 1:4 , x2 = 1:4 , x3 = 1:4 , x4 = 1:4)
  grid$std = NA
  grid$mean = NA
  for (i in  1:nrow(grid)) {
    grid[i,'mean'] = mean(c( (grid[i,]$x1) , (grid[i,]$x2)  , (grid[i,]$x3) , (grid[i,]$x4) ) )
    grid[i,'std'] = sd(c( (grid[i,]$x1) , (grid[i,]$x2)  , (grid[i,]$x3) , (grid[i,]$x4) ) )
  }
...
### ye demo na mescolata ... 
bootrap_factor = 4 
cat(">>> boostrap factor:",bootrap_factor,"...\n")

cat(">>> old train set:",dim(x[trind,]),"..\n")
x.train = x[trind,]
for (i in 1:(bootrap_factor-1)) {
...
y.train = train$median_relevance
for (i in 1:(bootrap_factor-1)) {
  y.train = c(y.train,train$median_relevance)
}

for (j in 1:length_init_train) {
  labels_j = get_labels(train[j,]$median_relevance , train[j,]$relevance_variance ,  bootrap_factor)
  y.train[j] = labels_j$x1 
  y.train[j+length_init_train] = labels_j$x2
  y.train[j+2*length_init_train] = labels_j$x3
  y.train[j+3*length_init_train] = labels_j$x4
}

y = y.train #### if no bootsrap: y[i] == y[i+10158] == y[i+2*10158]== y[i+3*10158] 
y = y-1 
