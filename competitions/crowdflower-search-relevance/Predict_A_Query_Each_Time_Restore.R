...
######################################### TODO
### nel caso in cui hai + early_stop scegli l'ultimo anzicche' il primo 
################################################################################## 
...
#   }
  ...
  train$merge = apply(X = train , 1 , function(x) paste(x[3] , x[4] , sep= ' ') )
  test$merge = apply(X = test , 1 , function(x) paste(x[3] , x[4] , sep= ' ') )
...
## restore 
pred = read_csv(paste(getBasePath("data") , "sub_dump.csv" , sep=''))$prediction
cv_grid = read_csv(paste(getBasePath("data") , "cv_grid_dump.csv" , sep=''))
last_query_before_crack = max(cv_grid[ ! is.na(cv_grid$train_obs) , ]$query)
cat(">>> restoring from query N.",last_query_before_crack,"...\n")

## continue the job 
for (i in last_query_before_crack:261) {
  cat(">>> processing query ",i,"...\n")
  train_idx = which(train$q_class == i) 
  test_idx = which(test$q_class == i) 
  ...
  ### update cv_grid 
  cv_grid[cv_grid$query == i , ]$train_obs = length(train_idx)
  cv_grid[cv_grid$query == i , ]$early_stop = early_stop
  cv_grid[cv_grid$query == i , ]$xval_qwk = xval_qwk
