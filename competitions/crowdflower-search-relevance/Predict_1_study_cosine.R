...
get_cosine_similarity = function(train,test,settings) {
  cp = myCorpus( c(train$query , train$product_title , test$query , test$product_title), 
  ...
  ##### Convert matrices 
  cat (">>> computing cosine similarity on 1-grams ... \n")
  x = as.matrix(dtm.tfidf.1)
  dtm.tfidf.1 = matrix(as.numeric(x),nrow(x),ncol(x))
  
  dtm.tfidf.1_query_train = dtm.tfidf.1[1:nrow(train),]
  dtm.tfidf.1_pt_train = dtm.tfidf.1[(nrow(train)+1):(2*nrow(train)),]
  dtm.tfidf.1_query_test = dtm.tfidf.1[(2*nrow(train)+1):(2*nrow(train)+nrow(test)),]
  dtm.tfidf.1_pt_test = dtm.tfidf.1[(2*nrow(train)+nrow(test)+1):(2*nrow(train)+2*nrow(test)),]
  dtm.tfidf.1_cosine = c(diag(dtm.tfidf.1_query_train %*% t(dtm.tfidf.1_pt_train)) , diag(dtm.tfidf.1_query_test %*% t(dtm.tfidf.1_pt_test)) ) 
  
  if ( length(dtm.tfidf.1_cosine) != (nrow(train)+nrow(test)) ) stop("something wrong with sti cazz the producttttt...")
  
  dtm.tfidf = dtm.tfidf.1_cosine
  
  rm(dtm.tfidf.1)
  rm(dtm.tfidf.1_query_train)
  rm(dtm.tfidf.1_pt_train)
  rm(dtm.tfidf.1_query_test)
  rm(dtm.tfidf.1_pt_test)
  
  if ( as.logical(settings[settings$variant == "bigrams" , ]$value) ) { 
    cat ("computing cosine similarity on 2-grams ...\n")
    x = as.matrix(dtm.tfidf.2)
    dtm.tfidf.2 = matrix(as.numeric(x),nrow(x),ncol(x))
    
    dtm.tfidf.2_query_train = dtm.tfidf.2[1:nrow(train),]
    dtm.tfidf.2_pt_train = dtm.tfidf.2[(nrow(train)+1):(2*nrow(train)),]
    dtm.tfidf.2_query_test = dtm.tfidf.2[(2*nrow(train)+1):(2*nrow(train)+nrow(test)),]
    dtm.tfidf.2_pt_test = dtm.tfidf.2[(2*nrow(train)+nrow(test)+1):(2*nrow(train)+2*nrow(test)),]
    dtm.tfidf.2_cosine = c(diag(dtm.tfidf.2_query_train %*% t(dtm.tfidf.2_pt_train)) , diag(dtm.tfidf.2_query_test %*% t(dtm.tfidf.2_pt_test)) )
    
    ##
    dtm.tfidf = cbind(dtm.tfidf ,  dtm.tfidf.2_cosine)
    
    rm(dtm.tfidf.2)
    rm(dtm.tfidf.2_query_train)
    rm(dtm.tfidf.2_pt_train)
    rm(dtm.tfidf.2_query_test)
    rm(dtm.tfidf.2_pt_test)
  }
  
  if ( as.logical(settings[settings$variant == "trigrams" , ]$value) ) { 
    cat ("computing cosine similarity on 3-grams ...\n")
    x = as.matrix(dtm.tfidf.3)
    dtm.tfidf.3 = matrix(as.numeric(x),nrow(x),ncol(x))
    
    dtm.tfidf.3_query_train = dtm.tfidf.3[1:nrow(train),]
    dtm.tfidf.3_pt_train = dtm.tfidf.3[(nrow(train)+1):(2*nrow(train)),]
    dtm.tfidf.3_query_test = dtm.tfidf.3[(2*nrow(train)+1):(2*nrow(train)+nrow(test)),]
    dtm.tfidf.3_pt_test = dtm.tfidf.3[(2*nrow(train)+nrow(test)+1):(2*nrow(train)+2*nrow(test)),]
    dtm.tfidf.3_cosine = c(diag(dtm.tfidf.3_query_train %*% t(dtm.tfidf.3_pt_train)) , diag(dtm.tfidf.3_query_test %*% t(dtm.tfidf.3_pt_test)) )
    
    ###
    dtm.tfid = cbind(dtm.tfidf ,  dtm.tfidf.3_cosine)
    
    rm(dtm.tfidf.3)
    rm(dtm.tfidf.3_query_train)
    rm(dtm.tfidf.3_pt_train)
    rm(dtm.tfidf.3_query_test)
    rm(dtm.tfidf.3_pt_test)
  }
  
  return(as.data.frame(dtm.tfid))
}

get_ngrams = function(train,test,settings) {
...
### 
dtm.tfidf.cosine = get_cosine_similarity(train,test,settings)
dtm.tfidf.df = get_ngrams (train,test,settings)

### binding other features 
cat ("> binding other features ... \n")

dtm.tfidf.df = cbind(dtm.tfidf.df,dtm.tfidf.cosine)
