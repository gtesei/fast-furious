...
#     testdata$trend = fit.base[test.idx]
   ...
   if (st == 37 & it ==5) {
    ...
    if ( perf.5 < rmse ) {
      cat(">> in this case Enet model performs better, so let's change submissions ... \n ")
      update.sub = T
      pred = pred.5
      grid[i,]$choice = 'enet'
    }
  ...
  if (update.sub)  {
    ...
    pred = ifelse(pred > 0 & pred < 1 & pred %% 1 < 0.5 , floor(pred) , pred )
    id = sub[grep(x = sub$id , pattern = paste("^",st,"_",it,"_",sep='') ), ]$id
...
write.csv(sub,quote=FALSE, 
          file=paste(getBasePath("data"),"sub2_adjust_enet__selPred.csv",sep='') ,
          row.names=FALSE)

write.csv(grid,quote=FALSE, 
          file=paste(getBasePath("data"),"grid2_adjust_enet.csv",sep='') ,
          row.names=FALSE)

cat("<<<<< submission correctly stored on disk >>>>>\n")

n.mod = sum(grid$choice == 'enet')
