...
#     testdata$trend = fit.base[test.idx]
   ...
   if (st == 37 & it ==5) {
    ...
    model.5 <- train(form, data = cbind(traindata,y=traindata.y) , 
                     method = "ridge",
                     tuneGrid = data.frame(.lambda = seq(0, .1, length = 25)),
...
write.csv(sub,quote=FALSE, 
          file=paste(getBasePath("data"),"sub_adjust_ridge__selPred.csv",sep='') ,
          row.names=FALSE)

write.csv(grid,quote=FALSE, 
          file=paste(getBasePath("data"),"grid_adjust_ridge.csv",sep='') ,
          row.names=FALSE)
