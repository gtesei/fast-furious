...
for (i in 1:num.adj) {
  ...
  l = tryCatch({ 
    rebuildSignal (train.chunk, test.chunk,doPlot=T,st=st,it=it,pred.orig)
  } , error = function(err) { 
  ...
  if (! is.null(l) ) { 
    ...
    stop("error: l is null!!")
  }
  
 
  cat("\n Enter a number >> (1) - pred is ok / (2) - pred is bad / (0) - exit  \n") 
  kb = scan(n=1)
  if (kb == 0) {
    break
  } else if (kb == 1) {
...
write.csv(sub,quote=FALSE, 
          file=paste(getBasePath("data"),"sub_adjust_man.csv",sep='') ,
          row.names=FALSE)
