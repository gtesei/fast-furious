...
predict.trainTS <- function(train.chunk, test.chunk,doPlot=F,st,it) {
  ...
  pred.train = as.numeric(fit[!is.na(x)])
  res = train.chunk$units - pred.train
  list(pred.train,res)
}
...
for (i in 1:dim(grid)[1]) {
  ...
  if (st == 37 & it == 5) {
    ...
    update.sub = T
    cat(" >>> BOOSTING <<<< \n")
    l = predict.trainTS (train.chunk, test.chunk,doPlot=T,st,it)
    pred.train = l[[1]]
    res = l[[2]]
