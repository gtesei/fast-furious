function [lambda_opt,J_opt] = ...
  findOptLambda_RegLinLiberty(Xtrain, ytrain, Xval, yval, lambda_vec = [0 0.001 0.003 0.01 0.03 0.1 0.3 1 3 10]' , p=5)
  ...
  for lambdaIdx = 1:length(lambda_vec)
        ...
        #[error_val(lambdaIdx), grad_cv] =      linearRegCostFunction(X_poly_val,   yval,   theta, 0);
                       ...
                       error_train(lambdaIdx) = NormalizedWeightedGini(ytrain,Xtrain(:,43),pred_train);
                       error_val(lambdaIdx) =   NormalizedWeightedGini(yval,Xval(:,43),pred_val);
  endfor
