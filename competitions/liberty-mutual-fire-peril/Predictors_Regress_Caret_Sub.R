...
#     pred = apply(pred.boot ,1,mean)
    ...
    fit <- train(x , y , 
                     ...
                     trControl = trainControl(method = "cv" , number = 10))
    fit
    pred = as.numeric( predict(fit , x.test )  )
  } else if (type == "m5") { 
    x = model.matrix ( as.formula(form)  , train )[,-1]
    x.test = model.matrix ( as.formula(form)  , cbind(target = 0 , test) ) [,-1]
    y = train$target
    ...
    fit <- train(as.formula(form) , data=train , method = "cubist")
    pred = as.numeric( predict(fit , test )  )
  } else if (type == "ctree2") {  
    ...
    fit <- train(x , y , method = "gbm", tuneGrid = gbmGrid, verbose = FALSE)
    fit 
    pred = as.numeric( predict(fit , x.test )  )
  }
...
##### MARS 
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "mars")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_mars.csv.gz" , subI)
...
##### BOOSTED TREES 
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "gbm")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_gbm.csv.gz" , subI)

##### RANDOM FOREST
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "rf")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_rf.csv.gz" , subI)

##### CUBIST
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "cubist")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_cubist.csv.gz" , subI)

## CONDITIONAL TREE 
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "ctree2")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_ctree2.csv.gz" , subI)

##### SVM 
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "svm")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_svm.csv.gz" , subI)

##### MODEL TREES 
pred.nona = trainAndPredict(form, train, test.nona[,-1], type = "m5")
subI = mergeTestSet (test.na , test.nona , 0 , pred.nona, sampleSub.tab) 
writeOnDisk(base.path , "sub_m5.csv.gz" , subI)
