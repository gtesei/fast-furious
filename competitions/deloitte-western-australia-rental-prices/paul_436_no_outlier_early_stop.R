...
###### XBG 
cat(">>> XBG training [MAX_NROUNDS=",MAX_NROUNDS,"]...\n")
...
## 
tra = x[trind,]
cat(">>> selecting cv obs:",ceiling(nrow(x[trind,])*CV_SIZE),"out of",nrow(x[trind,]),"[",CV_SIZE,"%] ... ") 
h<-sample(nrow(x[trind,]), ceiling(nrow(x[trind,])*CV_SIZE) )

dtrain <- xgboost::xgb.DMatrix(tra[-h,], label = Ytrain[-h])
dval <- xgboost::xgb.DMatrix(tra[h,], label = Ytrain[h])
...
write.csv(submission,
          quote=FALSE, 
          file=paste(ff.getPath("submission_1") , "paul_36_ES_removeOutliersITS_",CUTOFF,".csv" , sep='') ,
          row.names=FALSE)
