...
length(land_valuation_key$LAN_ID) == length(unique(land_valuation_key$LAN_ID)) ## FALSE 

## VE_NUMBER e' unique in land_valuation_key ? .. no 
length(land_valuation_key$VE_NUMBER) == length(unique(land_valuation_key$VE_NUMBER)) ## FALSE 
...
#stopifnot(nrow(Xtrain)==Xtrain.n_now)
#stopifnot(nrow(Xtest)==Xtest.n_now)
rm(valuation_entities_classifications)

### valuation_entities_details
cat(">>> valuation_entities_details ... \n")
...
sum(VE_NA %in% train$VE_NUMBER ) ## 95
sum(VE_NA %in% test$VE_NUMBER ) ## 15
...
## >> from dictionary it seems that a given variable can be valid in a given period of time 
##    so, if the valuation occurs in that priod of time, such a variable should be taken into account, and viceversa. 
##    anyway 94% of observations in valuation_entities_details has NAs in UVV_DATE_EFF_TO, 
##    i.e. how NAs should be treated? Notice that there are dates in the future (e.g. 2020-11-30)
##    >> I decide to take into account all observartions.  

## feat. select. 
valuation_entities_details$URV_DATE_EFF_FROM <- NULL
valuation_entities_details$UVV_DATE_EFF_FROM <- NULL
valuation_entities_details$UVV_DATE_EFF_TO <- NULL

## building a structure like <VE_NUMBER> , <# occ attr 1> , <# occ attr 2> .. 

head(unique(valuation_entities_details$UVV_QUALITY))
head(unique(valuation_entities_details$UVV_QUANTITY))

## Let's implement this contraint 
# URV_VEN_QUALITY_IND 	-	  Defines whether a quality attribute is required for the variable. 
#                           If Y and no quality is defined, the variable is not valid.
# URV_VEN_QUANTITY_IND  -		Defines whether a quantity attribute is required for the variable. 
#                           If Y and no quantity is defined, the variable is not valid.

nrow(valuation_entities_details[valuation_entities_details$URV_VEN_QUALITY_IND=="Y" & is.na(valuation_entities_details$UVV_QUALITY) , ]) #0
nrow(valuation_entities_details[valuation_entities_details$URV_VEN_QUANTITY_IND=="Y" & is.na(valuation_entities_details$UVV_QUANTITY) , ]) #7788

valuation_entities_details = 
  valuation_entities_details[! (valuation_entities_details$URV_VEN_QUANTITY_IND=="Y" & is.na(valuation_entities_details$UVV_QUANTITY)) , ]

## how many categories? ... 3 
sum(valuation_entities_details$URV_VEN_QUANTITY_IND=="Y" & valuation_entities_details$URV_VEN_QUALITY_IND=="Y") #0
sum(valuation_entities_details$URV_VEN_QUANTITY_IND=="Y" & valuation_entities_details$URV_VEN_QUALITY_IND=="N") #2777746
sum(valuation_entities_details$URV_VEN_QUANTITY_IND=="N" & valuation_entities_details$URV_VEN_QUALITY_IND=="Y") #448033
sum(valuation_entities_details$URV_VEN_QUANTITY_IND=="N" & valuation_entities_details$URV_VEN_QUALITY_IND=="N") #817940

head(valuation_entities_details[valuation_entities_details$URV_VEN_QUANTITY_IND=="Y" & valuation_entities_details$URV_VEN_QUALITY_IND=="Y",])
head(valuation_entities_details[valuation_entities_details$URV_VEN_QUANTITY_IND=="Y" & valuation_entities_details$URV_VEN_QUALITY_IND=="N",])
head(valuation_entities_details[valuation_entities_details$URV_VEN_QUANTITY_IND=="N" & valuation_entities_details$URV_VEN_QUALITY_IND=="Y",])
head(valuation_entities_details[valuation_entities_details$URV_VEN_QUANTITY_IND=="N" & valuation_entities_details$URV_VEN_QUALITY_IND=="N",])

## let's build a meta-dictionary 
meta.dict = ddply(valuation_entities_details, .(URV_ID,URV_VEN_QUALITY_IND,URV_VEN_QUANTITY_IND) , function(x) c(num = nrow(x)))
stopifnot(nrow(meta.dict)==length(unique(valuation_entities_details$URV_ID)))
meta.dict$type = NA
meta.dict$index = NA
for (i in 1:nrow(meta.dict)) {
  if (meta.dict[i,]$URV_VEN_QUALITY_IND=="Y" && meta.dict[i,]$URV_VEN_QUANTITY_IND=="N") {
    meta.dict[i,]$type <- 1 
  } else if (meta.dict[i,]$URV_VEN_QUALITY_IND=="N" && meta.dict[i,]$URV_VEN_QUANTITY_IND=="Y") {
    meta.dict[i,]$type <- 2
  } else if (meta.dict[i,]$URV_VEN_QUALITY_IND=="N" && meta.dict[i,]$URV_VEN_QUANTITY_IND=="N") {
    meta.dict[i,]$type <- 3
  } else stop("something wrong.")
  meta.dict[i,]$index <- i 
}
stopifnot(sum(is.na(meta.dict))==0)
meta.dict = meta.dict[,c("URV_ID","type","index")]

## let's build the structure 
ved.digest = data.frame(VE_NUMBER=unique(valuation_entities_details$VE_NUMBER))

ved.digest.beef = as.data.frame(matrix(rep(NA,nrow(ved.digest)*nrow(meta.dict)),ncol = nrow(meta.dict) , nrow = nrow(ved.digest)))
for (i in 1:nrow(meta.dict)) {
  colnames(ved.digest.beef)[meta.dict[i,]$index] <- paste0("URV_ID_",meta.dict[i,]$URV_ID)
  type <- meta.dict[i,]$type
  if (type==1) {
    ved.digest.beef[,i] <- "NO_VALUE"
  } else if (type==2) {
    ved.digest.beef[,i] <- 0
  } else if (type==3) {
    ved.digest.beef[,i] <- 0
...
ved.digest = cbind(ved.digest,ved.digest.beef)
rm(ved.digest.beef)
col_offset = 1
a = lapply(1:nrow(ved.digest),function(i){
  if (i==1) cat(">>> starting [",i,"/",nrow(ved.digest),"] .. \n")
  if (i %% 10000 == 0) cat(">>> [",i,"/",nrow(ved.digest),"] .. \n")
  
  ##
  ve <- ved.digest[i,]$VE_NUMBER
  valuation_entities_details_i <- valuation_entities_details[valuation_entities_details$VE_NUMBER==ve,]
  
  aa <- lapply(1:nrow(valuation_entities_details_i),function(j){
    urvid <- valuation_entities_details_i[j,]$URV_ID
    type <- meta.dict[meta.dict$URV_ID==urvid,]$type
    index = meta.dict[meta.dict$URV_ID==urvid,]$index  
    
    if (type==1) {
      val <- valuation_entities_details_i[j,]$UVV_QUALITY
      ved.digest[i,(index+col_offset)] <<- val
    } else if (type==2) {
      val <- valuation_entities_details_i[j,]$UVV_QUANTITY
      ved.digest[i,(index+col_offset)] <<- val
    } else if (type==3) {
      ved.digest[i,(index+col_offset)] <<- 1
    } else stop("something wrong.")
...
## categ 
feature.names = colnames(ved.digest)
for (f in feature.names) {
  if (class(ved.digest[,f])=="character") {
    cat(">>> ",f," is character \n")
    levels <- unique(ved.digest[,f])
    ved.digest[,f] <- as.integer(factor(ved.digest[,f], levels=levels))
  }
}

## some checks 
sum(! (ved.digest$VE_NUMBER %in% valuation_entities$VE_NUMBER) ) ## 0 
sum(! ( valuation_entities$VE_NUMBER %in% ved.digest$VE_NUMBER) ) ## 107 
VE_NA = setdiff( x = valuation_entities$VE_NUMBER , y = unique(ved.digest$VE_NUMBER))
sum(VE_NA %in% train$VE_NUMBER ) ## 95
sum(VE_NA %in% test$VE_NUMBER ) ## 15

## merge 
Xtrain = merge(x = Xtrain , y = ved.digest , by = "VE_NUMBER" , all.x = T , all.y = F)
Xtest = merge(x = Xtest , y = ved.digest , by = "VE_NUMBER" , all.x = T , all.y = F)
cat(">>> Xtrain:",dim(Xtrain),"\n")
cat(">>> Xtest:",dim(Xtest),"\n")
stopifnot(nrow(Xtrain)==Xtrain.n_now)
stopifnot(nrow(Xtest)==Xtest.n_now)
...
## imputing NAs 
sum(is.na(Xtrain)) # 48140 
sum(is.na(Xtest))  # 11620
...
for (pNA in predNAs.train) {
  uid <- as.numeric(substr(pNA, start=8, stop=nchar(pNA)))
  cat(">>> imputing NAs for URV_ID == ",uid,"...\n")
  type <- meta.dict[meta.dict$URV_ID==uid,]$type
  
  if (type==1) {
    Xtrain[,pNA][is.na(Xtrain[,pNA])] <- 0
    Xtest[,pNA][is.na(Xtest[,pNA])] <- 0
  } else if (type==2) {
    Xtrain[,pNA][is.na(Xtrain[,pNA])] <- 0
    Xtest[,pNA][is.na(Xtest[,pNA])] <- 0 
  } else if (type==3) {
...
write.csv(Xtrain,
          quote=FALSE, 
          file=paste0(ff.getPath("elab"),"Xtrain_NAs4.csv"),
          row.names=FALSE)
write.csv(data.frame(Ytrain=Ytrain),
          quote=FALSE, 
          file=paste0(ff.getPath("elab"),"Ytrain_NAs4.csv"),
          row.names=FALSE)
write.csv(Xtest,
          quote=FALSE, 
          file=paste0(ff.getPath("elab"),"Xtest_NAs4.csv"),
          row.names=FALSE)
