...
## ## ## ## ## SVMs
model.grid = build.grid(c(SVM_MEAN_SD_SCALED))  

## ## ## ## ## LOG
model.grid = rbind ( model.grid , build.grid(c(LOGISTIC_REG_MEAN_SD_SCALED)) )

## ## ## ## ## LDA
model.grid = rbind ( model.grid , build.grid(c(LDA_MEAN_SD_SCALED, LDA_QUANTILES_SCALED)) )

## ## ## ## ## PLSDA
model.grid = rbind ( model.grid , build.grid(c(PLSDA_MEAN_SD_SCALED,PLSDA_QUANTILES_SCALED)) )

## ## ## ## ## NSC
model.grid = rbind ( model.grid , build.grid(c(NSC_MEAN_SD_SCALED)) )

## ## ## ## ## NN
model.grid = rbind ( model.grid , build.grid(c(NN_QUANTILES_REDUCED) , ds=c(4,7)) )

## ## ## ## ## KNN
model.grid = rbind ( model.grid , build.grid(c(KNN_MEAN_SD_SCALED,KNN_QUANTILES_SCALED,KNN_MEAN_SD_REDUCED,KNN_QUANTILES_REDUCED)  ) )

## ## ## ## ## CT
model.grid = rbind ( model.grid , build.grid(c(CLASS_TREE_MEAN_SD_SCALED,CLASS_TREE_QUANTILES_SCALED) , ds=c(5)) )

## ## ## ## ## BOOST     <<<<<<<<<<
model.grid = rbind ( model.grid , build.grid(c(BOOSTED_TREE_QUANTILES_REDUCED) , ds=c(4,7)) )
model.grid = rbind ( model.grid , build.grid(c(BOOSTED_TREE_QUANTILES_SCALED) , ds=c(5)) )

## ## ## ## ## BAGGING
model.grid = rbind ( model.grid , build.grid(c(BAGGING_TREE_MEAN_SD,BAGGING_TREE_QUANTILES)  ))

## ## ## ## ## PM
model.grid = rbind ( model.grid , build.grid(c(PM_MEAN_SD_SCALED) ) )
