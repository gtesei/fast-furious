...
## ## ## ## ## SVMs 
model.grid = build.grid(c(SVM_MEAN_SD_SCALED, SVM_QUANTILES_SCALED, SVM_MEAN_SD_REDUCED) ,  pca=c(F) )  

## ## ## ## ## LOG
model.grid = rbind ( model.grid , build.grid(c(LOGISTIC_REG_MEAN_SD_SCALED, LOGISTIC_REG_QUANTILES_SCALED) , pca=c(F) )  )

## ## ## ## ## LDA
model.grid = rbind ( model.grid , build.grid(c(LDA_MEAN_SD_SCALED, LDA_MEAN_SD_REDUCED) , pca=c(F) )  )

## ## ## ## ## PLSDA
model.grid = rbind ( model.grid , build.grid(c(PLSDA_QUANTILES_SCALED, PLSDA_REG_QUANTILES_REDUCED) , pca=c(F) )  )

## ## ## ## ## NSC
model.grid = rbind ( model.grid , build.grid( c(NSC_REG_QUANTILES_REDUCED) , pca=c(F) )  )

## ## ## ## ## NN
model.grid = rbind ( model.grid , build.grid(c(NN_MEAN_SD_REDUCED,NN_QUANTILES_REDUCED) ,  pca=c(F) )  )

## ## ## ## ## KNN
model.grid = rbind ( model.grid , build.grid(c(KNN_MEAN_SD_SCALED) ,  pca=c(F) )  )

## ## ## ## ## CT
model.grid = rbind ( model.grid , build.grid(c(CLASS_TREE_MEAN_SD) , pca=c(F) )  )

## ## ## ## ## BOOST
model.grid = rbind ( model.grid , build.grid(c(BOOSTED_TREE_MEAN_SD,BOOSTED_TREE_MEAN_SD_SCALED,BOOSTED_TREE_QUANTILES_SCALED,BOOSTED_TREE_QUANTILES_REDUCED) , pca=c(F) )  )
