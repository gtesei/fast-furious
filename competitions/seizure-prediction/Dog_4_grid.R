...
## ## ## ## ## SVMs
model.grid = build.grid(c(SVM_MEAN_SD_SCALED, SVM_QUANTILES_SCALED, SVM_MEAN_SD_REDUCED , SVM_QUANTILES_REDUCED) ,  pca=c(F) )  
...
## ## ## ## ## PLSDA
model.grid = rbind ( model.grid , build.grid(c(PLSDA_MEAN_SD_REDUCED,PLSDA_REG_QUANTILES_REDUCED) , pca=c(F) )  )

## ## ## ## ## NSC
model.grid = rbind ( model.grid , build.grid( c(NSC_MEAN_SD_SCALED) , pca=c(F) )  )

## ## ## ## ## NN
model.grid = rbind ( model.grid , build.grid(c(NN_MEAN_SD_REDUCED,NN_QUANTILES_REDUCED) ,  pca=c(F) )  )

## ## ## ## ## KNN
model.grid = rbind ( model.grid , build.grid(c(KNN_QUANTILES_SCALED,KNN_QUANTILES_REDUCED) ,  pca=c(F) )  )

## ## ## ## ## CT
model.grid = rbind ( model.grid , build.grid(c(CLASS_TREE_MEAN_SD_REDUCED) , pca=c(F) )  )

## ## ## ## ## BOOST
model.grid = rbind ( model.grid , build.grid(c(BOOSTED_TREE_MEAN_SD_SCALED,BOOSTED_TREE_QUANTILES_SCALED,BOOSTED_TREE_MEAN_SD_REDUCED,BOOSTED_TREE_QUANTILES_REDUCED) , pca=c(F) )  )

## ## ## ## ## BAGGING      <<<<<<
#model.grid = rbind ( model.grid , build.grid(c(BAGGING_TREE_MEAN_SD) , ds=c(4) , pca=c(F)  ) )

## ## ## ## ## PM
model.grid = rbind ( model.grid , build.grid(c(PM_MEAN_SD_SCALED,PM_QUANTILES_SCALED) ,  pca=c(F) ) )
