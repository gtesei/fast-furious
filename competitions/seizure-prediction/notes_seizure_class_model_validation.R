...
getBasePath = function (type = "data" , ds="") {
  ...
  if (ds != "" ) {
    ret = paste0(paste0(ret,ds),"_digest_7gen/")
  }
  ret
} 
...
buildRelativeSpectralPowerFeatures = function(Xtrain_mean_sd,Xtest_mean_sd,Xtrain_quant,Xtest_quant,ytrain,NN=10,verbose) {
  ...
  for (idx.num in 1:NN) {
    for (idx.denum in 1:NN) {
      ...
      feat.mat[,idx.denum+NN*(idx.num-1)] =  ((Xtrain_mean_sd[,idx.delta.mean.asc[idx.num]]) / (Xtrain_mean_sd[,idx.delta.mean.desc[idx.denum]]))^-1
    }
  }
  ...
  if (verbose) {
    for (yy in 1:(min(length(new.features.train),10))) {
      cat("----------------------------------------------------------------- \n")
  ...
  for (idx.num in 1:NN) {
    for (idx.denum in 1:NN) {
      ...
      feat.mat[,idx.denum+NN*(idx.num-1)] =  ((Xtest_mean_sd[,idx.delta.mean.asc[idx.num]]) / (Xtest_mean_sd[,idx.delta.mean.desc[idx.denum]]))^-1
    }
  }
...
#                                  tm, grid = perf.grid,verbose=verbose, doPlot=doPlot)
  ...
  write.csv(perf.grid,quote=FALSE,file=paste0(getBasePath(),paste0(ds,"_perf_grid_class_6gen.csv")), row.names=FALSE)
