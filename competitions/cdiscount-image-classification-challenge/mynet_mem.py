...
def read_images_memory(bson_file, n_images, num_class,images_df,offsets_df,target_size=(180, 180)):
    #from scipy import misc

    image_shape = target_size + (3,)
    batch_x = np.zeros((n_images,) + image_shape, dtype=K.floatx())
    batch_y = np.zeros((len(batch_x), num_class), dtype=K.floatx())
    for i in range (n_images):
        ...
        #img = misc.imread(io.BytesIO(bson_img))
        #x = img
        # Preprocess the image.
        x = img_to_array(img)
        x = preprocess_image(x)
...
x_train, y_train = read_images_memory(train_bson_file, num_train_images, num_classes,train_images_df,train_offsets_df)
x_val, y_val = read_images_memory(train_bson_file, num_val_images, num_classes,val_images_df,train_offsets_df)
