...
validateModelList = function(modelList) {
  ...
  for (i in seq_along(modelList)) {
    ...
    fn = paste(modelList[[i]]$cluster_levs,'_',modelList[[i]]$model.label,useQty,removeHCP,useMaterialId,logFeat,power32Feat,logLabel,nr2000,nr3000,nr4000,nr5000,power025,power03,power32,
...
for (m in  seq_along(modelList) ) { 
  ...
  if (logFeat) {
    ...
    ## train 
    tmp = train_set[,-grep(pattern = 'cost|material_id|tube_assembly_id',x = colnames(train_set))]
    tmp [] = lapply(tmp,function(x) {
                          x[which(x<0)] = 1.5
                          return(x)
                        })
    tmp = log(tmp)
    train_set[,-grep(pattern = 'cost|material_id|tube_assembly_id',x = colnames(train_set))] = tmp
    
    ## test 
    tmp = test_set[,-grep(pattern = 'cost|material_id|tube_assembly_id',x = colnames(test_set))]
    tmp [] = lapply(tmp,function(x) {
      x[which(x<0)] = 1.5
      return(x)
    })
    tmp = log(tmp)
    test_set[,-grep(pattern = 'cost|material_id|tube_assembly_id',x = colnames(test_set))] = tmp
    rm(list = c('tmp'))
  }
