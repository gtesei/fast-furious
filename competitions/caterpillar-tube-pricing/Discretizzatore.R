...
cat(">>>> initial RMSE: " , RMSLE(pred = Xtrain$best_ensenble , obs = Xtrain$cost) , "\n")

###
prices = sort(unique(Xtrain$cost))

### 
disc_pred = rep(NA, nrow(best_enseble))
ll = lapply(seq_along(disc_pred) , function(i) {
  p_d = -1 
  p_i = best_enseble[i, ]$assemble
...
stopifnot(  sum(is.na(disc_pred)) == 0 )

pred_train_disc = disc_pred[1:nrow(Xtrain)]

cat(">>>> final RMSE: " , RMSLE(pred = pred_train_disc , obs = Xtrain$cost) , "\n")
...
cat(">>>> final RMSLE: " , RMSLE(pred = pred_train_disc , obs = Xtrain$cost) , "\n")

sum(pred_train_disc=Xtrain$cost)/length(Xtrain$cost)
#[1] 13.43332
