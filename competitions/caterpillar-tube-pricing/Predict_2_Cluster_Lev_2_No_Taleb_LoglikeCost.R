...
MIN_OBS_TRAIN_FOR_CLUTERING_DEEPER = 4
...
param <- list("objective" = "reg:linear",
              ...
              "nthread" = 10, 
              "min_child_weight" = 5 , # 1 previous 
              "scale_pos_weight" = 1.0 , # from forum params["scale_pos_weight"] = 1.0
...
for (cl in 1:length(material_ids)) {
  ...
  zeta_main_cluster = mean(train_set[train_set$material_id==material_ids[cl],'cost']) / sd(train_set[train_set$material_id==material_ids[cl],'cost'])
  cat("> zeta_main_cluster: ",zeta_main_cluster,"\n")
  ...
  if (material_ids[cl] == 'UNKNOWN' || nrow(train_set_cl_1) == 0) {
    ...
    for (qty in sub_cluster_test$quantity) {
      ...
      if (     (length(obs_train) == 0) || 
               (obs_train < MIN_OBS_TRAIN_FOR_CLUTERING_DEEPER) || 
               (zeta_train < zeta_main_cluster) ) {
      ...
      if ( (! go_deeper) || (length(sd_train) > 0 && !is.na(sd_train)) ) {
        if (go_deeper) {
          ...
          ## using sub_cluster 
          ## good lev_1 clusters: SP-0036 / SP-0019 / SP-0039 / SP-0008 / SP-0035 / SP-0041 / SP-0029
          GOOD_CLUSTER = c('SP-0036','SP-0019' ,'SP-0039' , 'SP-0008' , 'SP-0035' ,'SP-0041' ,'SP-0029')
          if ( material_ids[cl] %in% GOOD_CLUSTER ) {
            cat(">> material_id ",material_ids[cl]," is a good lev 1 cluster [",GOOD_CLUSTER,"] ==> using sub_cluster submission ... \n")
            pred[pred_cl_idx] = sub_cluster[pred_cl_idx,'cost']
          } else {
            cat(">> material_id ",material_ids[cl]," is NOT a good lev 1 cluster [",GOOD_CLUSTER,"] ==> using sub_base submission ... \n")
            pred[pred_cl_idx] = sub_base[pred_cl_idx,'cost']
          } 
        }
      } else {
...
write.csv(sample_submission,quote=FALSE, 
          file=paste(getBasePath("submission"),'sub_cluster_lev_2_loglike.csv',sep='') ,
          row.names=FALSE)

sample_submission$cost = pred_real 
write.csv(sample_submission,quote=FALSE, 
          file=paste(getBasePath("submission"),'sub_cluster_lev_2_loglike_real.csv',sep='') ,
          row.names=FALSE)
