...
ff.bindPath(type = 'submission' , sub_path = 'dataset/caterpillar-tube-pricing/') ## out
...
################# build features set  
sub_best = as.data.frame( fread(paste(ff.getPath("submission_old") , 
                                      "sub_best.csv" , sep=''))) 
...
# lambda = 0.1  ## clusrer n.1
# y = (y^lambda-1)/lambda   ##<<<<:::::::: box-cox transformation 
...
#####
cat(">>> training cubist model ... \n")
controlObject <- trainControl(method = "repeatedcv", repeats = 3, number = 8)
cubistGrid <- expand.grid(.committees = c(1, 5, 10, 50, 75, 100),
                          .neighbors = c(0, 1, 3, 5, 7, 9))

model <- train(y = y, x = train_set_cl ,
               method = "cubist", preProcess=c('expoTrans'), 
               tuneGrid = cubistGrid,
               trControl = controlObject)

cat(">>> xval RMSE:",min(model$results$RMSE),"\n")

pred_cl = as.numeric( predict(model , test_set_cl )  )

##pred_cl = exp(pred_cl)  ## <<<<:::::::: log^-1 transformation 
#pred_cl= (pred_cl*lambda+1)^(1/lambda) ## <<<<:::::::: box-cox^-1 transformation 
  
### merging 
cat(">>> merging predictions ... \n")
pred = sub_best$cost
pred[predTrain_cl_idx] = pred_cl
sample_submission$cost = pred
write.csv(sample_submission,quote=FALSE, 
          file=paste(ff.getPath("submission"),'sub_cluster_1_expoTrans.csv',sep='') ,
          row.names=FALSE)
