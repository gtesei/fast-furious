...
### FUNCS 
getMetaData = function(ensembles_dir,report_fn=NULL,top_perc=NULL,dontTakeInReport=NULL) {
  ...
  if (! is.null(report_fn)) {
    ...
    identical(mf,kpi$ensemble)
    ##  TO BE CONTINUED .... 
  }
  
  # if (useOnly4Plus) {
  #   cat(">>> using option useOnly4Plus ... \n")
  #   mf = list.files( ensembles_dir , pattern = '4_*|8_*')
  # }
  print(mf)
...
validateModelList = function(modelList) {
  ...
  for (i in seq_along(modelList)) {
    ...
    fn = paste(modelList[[i]]$cluster_levs,'_',modelList[[i]]$model.label,useQty,removeHCP,useMaterialId,logLabel,nr2000,nr3000,nr4000,nr5000,power025,power03,power32,lr,'.csv',sep='')
...
ff.bindPath(type = 'ensemble_out' , sub_path = 'dataset/caterpillar-tube-pricing/ensembles/ensemble_1',createDir = TRUE) ## out 
ff.bindPath(type = 'best_tune_out' , sub_path = 'dataset/caterpillar-tube-pricing/ensembles/best_tune_1',createDir = TRUE) ## out 
ff.bindPath(type = 'submission' , sub_path = 'dataset/caterpillar-tube-pricing/ensembles/pred_ensemble_1',createDir = TRUE) ## out 
...
#      weights = rep(1/4,4) , enseble_dir = 'ensemble_out' )
