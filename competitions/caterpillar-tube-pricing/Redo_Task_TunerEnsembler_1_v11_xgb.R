...
dpoly = function (x,n) {
  ...
  ##
  x.poly = cbind(x.poly,x.poly.2)
  
  return(x.poly)
}

removeEqualPreds = function(Xtrain,Xtest) {
  data = rbind(Xtrain,Xtest)
  
  ## remove equal predictors 
  ...
  colnames(data)[colToRemove]
  data = data[,-which(colToRemove)]
#   
#   Xtrain = data[1:nrow(Xtrain),]
#   Xtest = data[(nrow(Xtrain)+1):(nrow(Xtrain)+nrow(Xtest)),]
  return(
    list(
      Xtrain = Xtrain , 
      Xtest = Xtest 
...
validateModelList = function(modelList) {
  ...
  for (i in seq_along(modelList)) {
    ...
    power64 = ifelse(!is.null(modelList[[i]]$power64) && modelList[[i]]$power64 , '_power64' , '')
    ...
    fn = paste(modelList[[i]]$cluster_levs,'_eta001_',modelList[[i]]$model.label,useQty,removeHCP,useMaterialId,logFeat,taqtylev,tadiamlev,
               power32Feat,logLabel,sqrtLabel,nr1500,nr2000,nr2500,nr3000,nr3500,nr4000,nr5000,power025,power03,power32,power64,
...
# list(layer = 1 , useLayer = c(T) , cluster_levs = 1 , model.label = 'ws' ,  predFile = '1_ws.csv' , tune=F , 
#      ensembles = c('1_eta001_xgbTreeGTJ_useQty_useMaterialId_taqtylev_tadiamlev_power32_1.csv',
...
list(layer = 1 , useLayer = c(T) , cluster_levs = 1 , model.label = 'xgbTreeGTJ'  , pgrade=2, useQty=T, tune=T, taqtylev=T ,tadiamlev=T, 
      useMaterialId=T, power64=T , predFile = '1_xgbTreeGTJ_nr5000_power32.csv')
...
#                     '1_eta001_rlm_logFeat_logLabel_rmsleThreshold0229_0_1.csv',
#                     '1_ws_useQty_0_1.csv', 
#                     '1_eta0001_rlm_logFeat_logLabel_rmsleThreshold023_0_1.csv',
#                     '1_eta001_rlm_logFeat_logLabel_rmsleThreshold0227_0_1.csv',
#                     '1_v2_rlm_useL2_10_0_1.csv', 
#                     '1_v2_rlm_useL2_10_corrFilter0.1_1_1.csv'
#                     ) , 
#       weights = rep(1/7,7) , enseble_dir = 'ensemble_2' )
...
# list(layer = 2 , logFeat=T, logLabel=T, useLayer = c(F,T) , cluster_levs = 1 , model.label = 'xgbTreeGTJ'  , 
#      rmsleThreshold=0.227, tune=T, predFile = '4_rlm_useL2_10_0_1.csv')
...
################list(layer = 3 , useLayer = c(F,T,T) , cluster_levs = 1 , model.label = 'rlm'  , rmsleThreshold=0.225 , tune=F, predFile = '4_rlm_rmsleThreshold0253_0_1.csv')
...
for (m in  seq_along(modelList) ) { 
  ...
  power64 = ifelse(!is.null(modelList[[m]]$power64) && modelList[[m]]$power64 , TRUE , FALSE)
  ...
  if(power64) {
    ...
    stopifnot(!power025)
    stopifnot(!power03)
    cat(">>> using power64 option ... \n")
    stopifnot( length(train_set$cost) == nrow(train_set) )
    train_set$cost = train_set$cost^(1/64)
  }
...
#       foldList = createFoldsSameTubeAssemblyId(data = train_set_cl, nFolds = 8, repeats = 3, seeds=c(123,456,789))
      ...
      foldList = createFoldsSameTubeAssemblyId(data = train_set_cl, nFolds = 8, repeats = 1, seeds=c(123))
foldList.xgb = list()
for (iii in 1:8) {
foldList.xgb[[paste0("Fold",iii)]] = which(foldList[[1]] == iii)
}
...
resamples = makeResampleIndexSameTubeAssemblyId(foldList)
      ...
      if ( model.label == 'xgbTreeGTJ' && (nr1500 || nr2000 || nr2500 || nr3000 || nr3500 || nr4000 || nr5000) ) {
        ...
        if (!is.null(modelList[[m]]$pgrade)) {
          ...
          cat(">>> using removeEqualPreds ...\n")
          rml = removeEqualPreds(Xtrain = train_set_cl , Xtest = test_set_cl)
          train_set_cl = rml$Xtrain
          test_set_cl = rml$Xtest 
        }
    ...
    for (i in seq_along(res_list)) {
      ...
      if (power64) {
        predTrain[res_list[[i]]$predTrain_cl_idx] = (res_list[[i]]$ensemble$predTrain)^64
        predTest[res_list[[i]]$predTest_cl_idx] = (res_list[[i]]$ensemble$predTest)^64
      }
    }
