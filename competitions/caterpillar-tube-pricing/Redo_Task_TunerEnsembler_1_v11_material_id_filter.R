...
validateModelList = function(modelList) {
  ...
  for (i in seq_along(modelList)) {
    ...
    matidf=''
    if (!is.null(modelList[[i]]$matidf)) {
      corrFilter = paste0('_matidf',modelList[[i]]$matidf)
    }
    ...
    fn = paste(modelList[[i]]$cluster_levs,'_v2_',modelList[[i]]$model.label,useQty,removeHCP,useMaterialId,logFeat,taqtylev,tadiamlev,
               power32Feat,logLabel,sqrtLabel,nr1500,nr2000,nr2500,nr3000,nr3500,nr4000,nr5000,power025,power03,power32,
               useL2_10,pickEnsembles,rmsleThreshold,pgrade,corrFilter,matidf,lr,'.csv',sep='')
...
list(layer = 1 , useLayer = c(T) , cluster_levs = 1 , model.label = 'cubist'  , matidf='SP-0028', tune=T, taqtylev=T ,tadiamlev=T, 
     predFile = '1_xgbTreeGTJ_nr5000_power32.csv')
...
best_enseble_train = best_enseble[1:train_obs,]$assemble
best_enseble_test = best_enseble[(train_obs+1):(train_obs+test_obs),]$assemble
...
for (m in  seq_along(modelList) ) { 
  ...
  matidf = NULL
  if (! is.null(modelList[[m]]$matidf) ) {
    matidf = modelList[[m]]$matidf
  }
  ...
  idxts = NULL 
  if (! is.null(matidf)) {
    cat(">>> using matidf=",matidf," option ... \n")
    idxtr = which(train_set$material_id==matidf)
    idxts = which(test_set$material_id==matidf)
    train_set = train_set[idxtr,]
    test_set = test_set[idxts,]
    cat(">>>  train length:",length(idxtr),"  ... \n")
    cat(">>>  test length:",length(idxts),"  ... \n")
  } 
  ...
  if (identical('ensembleAddBias',modelList[[m]]$model.label)) { ##ensembleAddBias
    ...
    ## clustering 
    if (modelList[[m]]$cluster_levs > 1)  {
      stop("non testato in questa configurazione")
#       cls = cluster_by(predictor.train=data[[1]]$traindata[,'quantity'],
#                        predictor.test=data[[1]]$testdata[,'quantity'],
#                        num_bids = modelList[[m]]$cluster_levs,
#                        verbose=T)
...
#       }
    ...
    if (tadiamlev) {
      ...
      cls = cluster_by_diamter(predictor.train=train_set$diameter,
                               predictor.test=test_set$diameter,
                               num_bids = 4,
                               verbose=T)
    ...
    for (i in seq_along(res_list)) {
      ...
      cat(">>> prediction length:",length(res_list[[i]]$pred_mod),"\n")
      predTest[idxts] = res_list[[i]]$pred_mod
    }
    ...
    for (i in seq_along(res_list)) {
      ...
      predTrain[idxtr] = res_list[[i]]$ensemble$predTrain
      predTest[idxts] = res_list[[i]]$ensemble$predTest
      if (sqrtLabel) {
        predTrain[idxtr] = (res_list[[i]]$ensemble$predTrain)^2
        predTest[idxts] = (res_list[[i]]$ensemble$predTest)^2
      }
      if (logLabel) {
        predTrain[idxtr] = exp(res_list[[i]]$ensemble$predTrain)
        predTest[idxts] = exp(res_list[[i]]$ensemble$predTest)
      }
      if (power025) {
        predTrain[idxtr] = (res_list[[i]]$ensemble$predTrain)^16
        predTest[idxts] = (res_list[[i]]$ensemble$predTest)^16
      }
      if (power03) {
        predTrain[idxtr] = (res_list[[i]]$ensemble$predTrain*(-0.3)+1)^(1/-0.3)
        predTest[idxts] = (res_list[[i]]$ensemble$predTest*(-0.3)+1)^(1/-0.3)
      }
      if (power32) {
        predTrain[idxtr] = (res_list[[i]]$ensemble$predTrain)^32
        predTest[idxts] = (res_list[[i]]$ensemble$predTest)^32
      }
    }
