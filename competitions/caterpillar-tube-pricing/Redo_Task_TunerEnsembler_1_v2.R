...
validateModelList = function(modelList) {
  ...
  for (i in seq_along(modelList)) {
    ...
    fn = paste(modelList[[i]]$cluster_levs,'_',modelList[[i]]$model.label,useQty,removeHCP,useMaterialId,logLabel,nr2000,nr3000,nr4000,power025,lr,'.csv',sep='')
...
for (m in  seq_along(modelList) ) { 
  ...
  res_list = parallel::mclapply( 1:modelList[[m]]$cluster_levs , function(i) {
    ...
    l = NULL
    if ( model.label == 'xgbTreeGTJ' && (nr2000 || nr3000 || nr4000) ) {
      nrounds = ifelse(nr2000,2000,ifelse(nr3000,3000,ifelse(nr4000,4000,stop("nround??"))))
