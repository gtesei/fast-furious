...
subfn = "base2_eta001_val_"+str(model.best_score)+"__rnd_"+str(model.best_iteration)+".csv"
...
pred = (pred<1)*pred+(pred>=1)*1
test[meta['target']] = pred 
subfn = "base2_eta001_all_data__rnd_"+str(model.best_iteration)+".csv"
test[[meta['test_id'], meta['target']]].to_csv(subfn, index=False)

xgb_feat_importance(model=model,cols=all_data.columns,file_name="base2_feat_importance_eta001.csv")
#################################################################################
