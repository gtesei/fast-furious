...
for gby_cols in [['center_lat_bin', 'center_long_bin'],
                 ...
                 ['pickup_month','pickup_hour','pickup_cluster', 'dropoff_cluster'],
...
do_not_use_for_training = ['id', 'log_trip_duration', 'pickup_datetime', 'dropoff_datetime', 'trip_duration', 'check_trip_duration',
                           ...
                           'center_lat_bin', 'center_long_bin', 'pickup_dt_bin', 'pickup_datetime_group', 'store_and_fwd_flag']
feature_names = [f for f in train.columns if f not in do_not_use_for_training]
print(feature_names)
print('We have %i features.' % len(feature_names))
train[feature_names].count()

#print('Removing outliers:'+str(np.sum(train.trip_duration > 1939735))+' ...')
#print('train shape BEFORE:'+str(train.shape))
#train = train.loc[train.trip_duration < 1939735] # <<< 
...
#model = xgb.train(xgb_pars, dtrain, 6000, watchlist, feval=rmsle_eval, early_stopping_rounds=50,maximize=False, verbose_eval=10)
model = xgb.train(params=xgb_pars, dtrain=dtrain, num_boost_round=1237, evals=(), feval=rmsle_eval,maximize=False, verbose_eval=10)



print('4. ---> Error ... ')
ypred = model.predict(dvalid)
print('Valid shape OK.') if Xv.shape[0] == ypred.shape[0] else print('Oops')
valid = pd.DataFrame(data=Xv,columns=feature_names)
valid['ypred'] = np.exp(ypred) - 1
valid['trip_duration'] = yv
#test['trip_duration'] = ytest
_ , rmsle_err = rmsle_eval_raw(y=yv, y0=ypred)
subfn = "error__rmsle_"+str(rmsle_err)+".csv.gz"
valid.to_csv(subfn, index=False, compression='gzip')
...
subfn = "error__rmsle_"+str(rmsle_err)+"feat_importance.csv" 
