...
svr = GridSearchCV(SVR(kernel='rbf', gamma=0.1), cv=4,
                   param_grid={'kernel': ['rbf','linear'], "C": [1e0, 1e1, 1e2, 1e3],
                               "gamma": np.logspace(-2, 2, 5)}, n_jobs=10, iid=False)

kr = GridSearchCV(KernelRidge(kernel='rbf', gamma=0.1), cv=4,
                  param_grid={'kernel': ['rbf','linear'], "alpha": [1e0, 0.1, 1e-2, 1e-3],
                              "gamma": np.logspace(-2, 2, 5)}, n_jobs=10, iid=False)


## svr
svr.fit(all_data[:train_obs], y_train)
print("SVR best params:",svr.best_params_)
svr_best_idx = np.argmax(svr.cv_results_['mean_test_score'])
print("SVR best score:", svr.cv_results_['mean_test_score'][svr_best_idx] , " - std:",svr.cv_results_['std_test_score'][svr_best_idx])
sys.stdout.flush()
svr_hypp = ''
for k in svr.best_params_.keys(): 
    svr_hypp = svr_hypp + "_"+str(k)+"_"+str(svr.best_params_[k])+"_" 

print('--------------> Submission ... ')
#svr 
test['log_price'] = svr.predict(all_data[train_obs:])
subfn = "base3dev1__svr__val_"+str(svr.cv_results_['mean_test_score'][svr_best_idx])+svr_hypp+".csv"
test[['id', 'log_price']].to_csv(subfn, index=False)

## kr
kr.fit(all_data[:train_obs], y_train)
print("KRR best params:",kr.best_params_)
krr_best_idx = np.argmax(kr.cv_results_['mean_test_score'])
print("KRR best score:", kr.cv_results_['mean_test_score'][krr_best_idx] , " - std:",kr.cv_results_['std_test_score'][krr_best_idx])
sys.stdout.flush()
kr_hypp = ''
for k in kr.best_params_.keys(): 
    kr_hypp = kr_hypp + "_"+str(k)+"_"+str(kr.best_params_[k])+"_" 

print('--------------> Submission ... ')
#kr 
test['log_price'] = kr.predict(all_data[train_obs:])
subfn = "base3dev1__kr__val_"+str(kr.cv_results_['mean_test_score'][krr_best_idx])+kr_hypp+".csv"
test[['id', 'log_price']].to_csv(subfn, index=False)
