...
model = xgb.train(xgb_pars, dtrain, 700, maximize=False, verbose_eval=10)

print('--------------> Submission ... ')
test['log_price'] = model.predict(dtest)
subfn = "base1__alldata__rnd_700.csv.gz"
test[['id', 'log_price']].to_csv(subfn, index=False, compression='gzip')
...
subfn = "error__feat_importance.csv" 
