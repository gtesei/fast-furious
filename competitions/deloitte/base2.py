...
all_data.loc[:, 'geo_cluster'] = kmeans.predict(all_data[['latitude','longitude']])
...
all_data = all_data.drop(['name','neighbourhood','thumbnail_url','zipcode'],axis=1)
...
subfn = "base2__val_"+str(model.best_score)+"__rnd_"+str(model.best_iteration)+".csv"
...
dtest = xgb.DMatrix(all_data[train_obs:].values)
model = xgb.train(xgb_pars, dtrain, model.best_iteration, maximize=False, verbose_eval=10)
print('--------------> Submission ... ')
test['log_price'] = model.predict(dtest)
subfn = "base2__all_data__rnd_"+str(model.best_iteration)+".csv"
test[['id', 'log_price']].to_csv(subfn, index=False)
...
subfn = "error__feat_importance_base2.csv" 
