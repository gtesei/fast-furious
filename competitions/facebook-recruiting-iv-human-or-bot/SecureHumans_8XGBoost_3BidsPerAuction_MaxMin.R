...
avg_bids_per_auction = ddply(bids_per_auction , .(bidder_id) , function(x) c(
  avg_bids_per_auction = sum(x$bid.num)/length(x$auction) 
  , min_bids_per_auction = min(x$bid.num) 
  , max_bids_per_auction = max(x$bid.num) 
  ) )

bidder_bids_per_auction = as.data.frame(matrix(rep(0,nrow(X)*2),nrow=nrow(X),ncol=2))
colnames(bidder_bids_per_auction) = c("avg_bids","min_bids")
...
for (bid in (1:max(teind))) {
  ...
  #bidder_bids_per_auction[bid,'max_bids'] = avg_bids_per_auction[avg_bids_per_auction$bidder_id == bidder_id,]$max_bids_per_auction
  bidder_bids_per_auction[bid,'min_bids'] = avg_bids_per_auction[avg_bids_per_auction$bidder_id == bidder_id,]$min_bids_per_auction
