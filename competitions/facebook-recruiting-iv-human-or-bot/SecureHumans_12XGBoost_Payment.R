...
#################
cat(">>> In train set there are ",length(unique(substr(train$payment_account , start=1, stop=32))) ," pay out of ",nrow(train),"\n")
cat(">>> In test set there are ",length(unique(substr(test$payment_account , start=1, stop=32))) ," pay out of ",nrow(test),"\n")

pay_train = ddply(train, .(bidder_id)  , function (x)  c(pay = substr(x$payment_account , start=1, stop=32)) )
pay_test = ddply(test, .(bidder_id)  , function (x)  c(pay = substr(x$payment_account , start=1, stop=32)) )

pay = rbind(pay_train,pay_test)

cat(">>> making features ...\n")
cat(">>> dim X BEFORE merge:",dim(X),"...\n")
XX = merge(x=X,y=pay,by='bidder_id')
cat(">>> dim X AFTER merge:",dim(X),"...\n")

cat(">>> encoding ...\n")
l = encodeCategoricalFeature (XX[,'pay'] , NULL , colname.prefix = "pay" , asNumericSequence=F, replaceWhiteSpaceInLevelsWith='_')
tr = l[[1]]
ts = l[[2]]
...
## writing on disk 
fn = paste("sub_xgboost_pay_xval" , xval.perf , ".csv" , sep='') 
