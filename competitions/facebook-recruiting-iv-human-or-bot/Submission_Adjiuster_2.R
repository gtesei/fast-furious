...
M4 = merge(x=X[X$merchandise_computers > 0 , ] 
           , y = avg_bids , by = 'bidder_id')

bidder_1_auction_small = M3[M3$avg_bids < 2 & M3$auction_num < 10 & M3$bids_num < 5000,]$bidder_id ## AUC = 0.920
#bidder_1_auction_small = M3[M3$avg_bids < 2 & M3$auction_num < 25 & M3$bids_num < 25000,]$bidder_id ## AUC = 0.914 
...
####
bidder_auction_big = M4[M3$avg_bids > 100 & M3$auction_num > 300 & M3$bids_num > 10000,]$bidder_id 

table(train[train$bidder_id %in% M4$bidder_id , ]$outcome)/length(train[train$bidder_id %in% M4$bidder_id , ]$outcome)

table(train[train$bidder_id %in% bidder_auction_big , ]$outcome)/length(train[train$bidder_id %in% bidder_auction_big , ]$outcome)
length(train[train$bidder_id %in% bidder_auction_big & train$outcome == 0, ]$bidder_id)
length(train[train$bidder_id %in% bidder_auction_big & train$outcome == 1, ]$bidder_id)

length(sub_best[sub_best$bidder_id %in% bidder_auction_big , ]$prediction) 
mean(sub_best[sub_best$bidder_id %in% bidder_auction_big , ]$prediction)
max(sub_best[sub_best$bidder_id %in% bidder_auction_big , ]$prediction)
min(sub_best[sub_best$bidder_id %in% bidder_auction_big , ]$prediction)
