...
i = 1
for num_leaves in [13,10,16]:
    for learning_rate in [0.0083,0.005]:
        for feature_fraction in [0.02,0.03]:
            for bagging_fraction in [0.5,0.7]:
                for scale_pos_weight in [1.0,scale_pos_weight_train]:
                    for max_depth in [8,10,12]:
                        for bagging_freq in [3,5,7]:
                            for min_data_in_leaf in [100,120]:
                                for min_sum_hessian_in_leaf in [10.0]:
                                    ...
                                    grid.to_csv('base_grid_lightGBM_2.csv',index=False)
