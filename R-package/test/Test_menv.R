...
if (exists("ff_test_param$verbose") && ff_test_param$verbose) cat(">>> Testing menv.R ... \n")
...
checkEquals(length(ff.get_path_bindings()),1)

ff.bind_path(type = "data",sub_path = "dataset")
checkEquals(length(ff.get_path_bindings()),2)
...
checkEquals(length(ff.get_path_bindings()),3)
checkEquals(ff.get_path('code') , paste(ff.get_path(),'data_process',.Platform$file.sep,sep = '') )

if (exists("ff_test_param$verbose") && ff_test_param$verbose) cat(">>> End of menv.R testing\n")
