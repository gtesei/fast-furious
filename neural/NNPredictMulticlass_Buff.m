function [y] = NNPredictMulticlass_Buff(NNMeta,fX,ciX,ceX,Theta,b=10000,_sep=',',featureScaled = 0)
 ...
 for i = 1:L-1
  ...
  elseif (i == 1)
    h(i,1) = sigmoid([ones(c, 1) X] * cell2mat(Theta(i,1))');
  else 
    h(i,1) = sigmoid([ones(c, 1) cell2mat(h(i-1,1))] * cell2mat(Theta(i,1))');
  endif
 endfor 
 ...
 while ((size(X,1) == b) && (c < m) )
   ...
   for i = 1:L-1
    if (featureScaled & i == 1)
      h(i,1) = sigmoid(X * cell2mat(Theta(i,1))');
    elseif (i == 1)
      h(i,1) = sigmoid([ones(_b, 1) X] * cell2mat(Theta(i,1))');
    else 
      h(i,1) = sigmoid([ones(_b, 1) cell2mat(h(i-1,1))] * cell2mat(Theta(i,1))');
    endif
   endfor 
