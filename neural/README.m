...
function [is_ok] = var1_doBasicUseCaseReg()

global curr_dir;

 is_ok = 0; % return as 1 if ok

 printf("Running var1_doBasicUseCaseReg ... \n");
 ...
 [Xtrain,mu,sigma] = treatContFeatures(_Xtrain,4);
 [Xval,mu_val,sigma_val] = treatContFeatures(_Xval,4,1,mu,sigma);

 [theta] = trainLinearReg(Xtrain, ytrain, 0 , 200 );
...
###### NN reg
 ...
 [Theta] = trainNeuralNetworkReg(NNMeta, Xtrain, ytrain, 0 , iter = 450, featureScaled = 1 );
